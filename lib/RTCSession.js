...
module.exports = class RTCSession extends EventEmitter
{
  ...
  _createRTCConnection(pcConfig, rtcConstraints)
  {
    ...
    this._connection.addEventListener('iceconnectionstatechange', () =>
    {
      ...
      if (this._ua.modes.chrome_rtp_timeout_fix !== undefined && navigator.webkitGetUserMedia && state === 'disconnected'){
        let timeout = this._ua.modes.chrome_rtp_timeout_fix;
    ...
    return Promise.resolve()
      ...
      .then(() =>
      {
        ...
        return new Promise((resolve) =>
        {
          ...
          if(this._ua.modes.ice_timeout_fix !== undefined){
              setTimeout(() => {
                  if (finished) return;
                  debug('AC: ICE gathering timeout fix');
                  ready();
              }, this._ua.modes.ice_timeout_fix); 
          }
          // end of fix
