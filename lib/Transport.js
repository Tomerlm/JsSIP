...
module.exports = class Transport
{
  ...
  _onDisconnect(error, code, reason)
  {
    ...
    if( this._ua.modes.sbc_ha_pairs_mode !== undefined ){
        if(prevStatus === C.STATUS_CONNECTED){
            debug('AC: SBC HA pairs mode: reconnecting to previous socket [during: ' + this._ua.modes.sbc_ha_pairs_mode + ' seconds]');
            this._ha_connect_time = new Date().getTime() + this._ua.modes.sbc_ha_pairs_mode * 1000; 
            this._connect2();
            return;
        }
        if(this._ha_connect_time !== undefined ){
            if( new Date().getTime() < this._ha_connect_time ){
                debug('AC: SBC HA pairs mode: reconnecting to previous socket [repeat]');
                this._connect2();
                return;
            } else {
                debug('AC: SBC HA pairs mode: cannot reconnect to previous socket');
                this._ha_connect_time = undefined;
            }
        }
    }
  ...
  _connect2(){
    ...
    if( this._ha_last_connect ){
		let elapsed = new Date().getTime() - this._ha_last_connect;
		delay = (elapsed > MinDelay) ? 0 : MinDelay - elapsed;
	} 
	debug('AC: SBC HA pairs mode: _connect2(): wait delay=', delay);

	this.recovery_timer = setTimeout(() => {
      if(!this.close_requested && !(this.isConnected() || this.isConnecting())){
		this._ha_last_connect = new Date().getTime();
		debug('AC: SBC HA pairs mode: _connect2(): call connect()');
        this.connect();
      }
    }, delay);
  }
